...
CREATE TABLE IF NOT EXISTS addresses (
    ...
    metadata JSONB, -- JSON object for additional address information (e.g., category, notes)
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
    FOREIGN KEY (organization_id) REFERENCES organizations(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS payment_methods (
    ...
    metadata JSONB, -- JSON object for additional payment method information (e.g., category, notes)
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
    FOREIGN KEY (organization_id) REFERENCES organizations(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS invoices (
    ...
    line_items JSONB, -- JSON array with detailed line item breakdown
...
CREATE TRIGGER update_product_costs_timestamp
...
CREATE TABLE IF NOT EXISTS order_costs (
    ...
    cost_breakdown JSONB, -- JSON with detailed breakdown
...
SELECT
    ...
    CASE
        ...
        THEN ROUND(((o.total_cents - COALESCE(SUM(oc.total_cost_cents), 0)) * 100.0 / o.total_cents)::numeric, 2)
        ELSE 0
...
SELECT
    ...
    CASE
        ...
        THEN ROUND(((s.total_cents - COALESCE(SUM(oc.total_cost_cents), 0)) * 100.0 / s.total_cents)::numeric, 2)
        ELSE 0
    END as profit_margin_percentage,
    s.billing_interval,
    s.created_at
...
CREATE TABLE IF NOT EXISTS receipts (
    ...
    customer_address JSONB, -- JSON with address details
    ...
    -- Metadata
    line_items JSONB, -- JSON with item breakdown
    applied_coupons JSONB, -- JSON with applied coupons
    notes TEXT,
    metadata JSONB,
